...
MTXInterface* MT3Interface::getinterface(int id)
{
	...
	switch (id){
	case 0: return (MTXInterface*)instance;
	...
	MTTRY{
		...
		for (x=0;x<16;x++){
			...
			if ((&cmodule) && (!cmodule.lockread)){
				if (cmodule.cpu) tot += cmodule.cpu->getcpu(0);
...
void MT3Interface::notify(void *object,int notify,int param)
{
	...
	case MTN_NEW:
		if (cobject.parent) cobject.parent->setmodified(1,(cobject.objecttype & MTO_TYPEMASK)|MTOM_ADDDELETE);
		break;
	};
	LEAVE();
}
...
bool MT3Interface::editobject(void *object,bool newwindow)
{
	...
	if (!window){
		...
		if ((tabs) && (tabs->guiid==MTC_TABCONTROL)) tabs->setautohidetabs(true);
		created = true;
	};
	if (oi->editobject(&cobject,window)){
		...
		if (created) gi->deletewindow(window);
		LEAVE();
		return false;
	};
}
...
void MT3Interface::delrefreshproc(int id)
{
	...
	LOGD("%s - [MT3] WARNING: Procedure not found!"NL);
	LEAVE();
}
