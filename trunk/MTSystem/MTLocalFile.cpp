...
void *MTLocalFile::getpointer(int offset,int size)
{
	int waccess = 0;
		...
		if (maccess & MTF_READ) waccess |= PROT_READ;
		if (maccess & MTF_WRITE) waccess |= PROT_WRITE;
		if (allocalign==0) allocalign = getpagesize();
		mapoffset = (offset/allocalign)*allocalign;
		mapoffset = offset-mapoffset;
...
bool MTLocalFile::settime(int *modified,int *accessed)
{
		...
		fstat(fileno(fs),&s);
		if (*modified) utb.modtime = *modified;
		else utb.modtime = s.st_mtime;
...
bool MTLocalFolder::getfile(const char **name,int *attrib,double *size)
{
	char tmp[512];
		...
		if (name) *name = de->d_name;
		if (attrib){
			if (de->d_name[0]=='.'){
				if ((strcmp(de->d_name,".")==0) && (strcmp(de->d_name,"..")==0)) *attrib |= MTFA_HIDDEN;
			};
		};
		if ((attrib) || (size)){
			struct stat s;
			...
			stat(tmp,&s);
			if (attrib){
				if ((s.st_mode & S_IFMT)==S_IFDIR) *attrib |= MTFA_FOLDER;
			};
