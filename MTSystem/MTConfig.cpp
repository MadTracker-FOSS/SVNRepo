...
MTConfigFile* mtconfigfind(const char *filename)
{
	...
	if (mtfileexists((char*)filename)) return new MTConfigFile(filename);
	MTPreferences *prefs;
...
bool MTConfigFile::getparameter(const char *paramname,void *value,int desiredtype,int size)
{
	...
	if ((!f) || (sectionpos<0)) return false;
...
skip:
		...
		*v++ = 0;
		if (strcmp(p,paramname)) continue;
		while ((*v==' ') || (*v=='\t') || (*v==':') || (*v=='=')){
			v++;
			if (*v==0) return false;
		};
...
ok:
	...
	case MTCT_SINTEGER:
		tmpi = atoi(v);
		if (size==sizeof(char)) *(char*)value = (char)tmpi;
		else if (size==sizeof(short)) *(short*)value = (short)tmpi;
		else if (size==sizeof(int)) *(int*)value = tmpi;
		else return false;
		break;
	case MTCT_UINTEGER:
		tmpl = atol(v);
		if (size==sizeof(char)) *(unsigned char*)value = (unsigned char)tmpl;
		else if (size==sizeof(short)) *(unsigned short*)value = (unsigned short)tmpl;
		else if (size==sizeof(int)) *(unsigned int*)value = tmpl;
		else return false;
		break;
	case MTCT_FLOAT:
		tmpd = atof(v);
		if (size==sizeof(float)) *(float*)value = (float)tmpd;
		else if (size==sizeof(double)) *(double*)value = tmpd;
		else return false;
		break;
	case MTCT_BOOLEAN:
		if ((v[0]=='1') || (v[0]=='y') || (v[0]=='Y') || (v[0]=='t') || (v[0]=='T')) *(bool*)value = true;
...
exit:
	if ((line[0]!=0) && (f->pos()>0)) f->write(NL,sizeof(NL)-1);
		...
		if (np){
			...
			for (x=cnp;x<np->nitems;x++){
				...
				if (mnp.line==cline){
					...
					while (x-->0){
						if (f->eof()) goto exit;
						if (f->readln(line,256)) lastline = ++cline;
						else cline++;
					};
					strncpy(line,mnp.buf,256);
					cnp++;
...
exit:
	...
	if (fmt[0]==0){
		...
		strcat(fmt,"\t");
		if (strlen(paramname)<8) strcat(fmt,"\t");
	};
	switch (type){
	case MTCT_SINTEGER:
		strcat(fmt,"%d");
		if ((v) && (*v)) strcat(fmt,v);
		if (size==sizeof(char)) sprintf(line,fmt,*(char*)value);
		else if (size==sizeof(short)) sprintf(line,fmt,*(short*)value);
		else if (size==sizeof(int)) sprintf(line,fmt,*(int*)value);
		break;
	case MTCT_UINTEGER:
		strcat(fmt,"%u");
		if ((v) && (*v)) strcat(fmt,v);
		if (size==sizeof(char)) sprintf(line,fmt,*(unsigned char*)value);
		else if (size==sizeof(short)) sprintf(line,fmt,*(unsigned short*)value);
		else if (size==sizeof(int)) sprintf(line,fmt,*(unsigned int*)value);
		break;
	case MTCT_FLOAT:
		strcat(fmt,"%f");
		if ((v) && (*v)) strcat(fmt,v);
		if (size==sizeof(float)) sprintf(line,fmt,*(float*)value);
		else if (size==sizeof(double)) sprintf(line,fmt,*(double*)value);
		break;
	case MTCT_BOOLEAN:
		strcat(fmt,"%s");
		if ((v) && (*v)) strcat(fmt,v);
	...
	if (rnp){
		...
		if ((!found) && (cpos==f->length())){
			...
			if (!np) np = new MTArray(4,sizeof(NP));
			np->additems(cnp,1);
			NP &mnp = ((NP*)np->d)[cnp];
