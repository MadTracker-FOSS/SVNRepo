...
void MTPattManager::ondraw(MTRect &rect)
{
	...
	if (!&rect){
		...
		if (r.right>r.left){
			...
			if (x<0) x = 0;
			if ((rect.top<patty) && (rect.bottom>=0)) drawtrack(x,w-x+1);
			r.top = patty;
			r.bottom = patty+ch;
...
bool MTPattManager::onmessage(MTCMessage &msg)
{
	...
	case MTCM_MOUSEDOWN:
		...
		if (msg.y<patty){
			...
			if ((mp2.x>=tw) && (mp2.x<tw+bh*3+6)){
				if (mp2.x<tw+bh+2) patt->tracks[mp.x].on ^= true;
				else if (mp2.x<tw+bh*2+4) patt->tracks[mp.x].solo ^= true;
				else patt->tracks[mp.x].rec ^= true;
				if( parent->parent){
	...
	case MTCM_MOUSEUP:
		...
		break;
	case MTCM_MOUSEMOVE:
		if (parent->processmessage(msg)) return true;
...
void MTPattManager::setcursor(int l,int t,int p)
{
	...
	if ((cursor.x!=t) || (cursor.y!=l) || (cpos!=p)){
		if (cpos!=p){
			if (p<0){
				...
				if (++ccol==patt->tracks[cursor.x].ncolumns){
					t++;
					if (t>=patt->ntracks) t %= patt->ntracks;
					ccol = 0;
				};
			}
			else{
		...
		if (l>=patt->nlines) l %= patt->nlines;
		cursor.x = t;
		cursor.y = l;
		if (parent->parent){
		...
		if (t<otrack) setotrack(t);
		else if (t>=otrack+ntracks) setotrack(t-ntracks+1);
		if (l<oline) setoline(l);
		else if (l>=oline+nlines) setoline(l-nlines+1);
		if (parent->parent){
...
{
	if (patt){
		if (value>patt->ntracks-ntracks) value = patt->ntracks-ntracks;
		if (value<0) value = 0;
	}
	else{
		value = 0;
...
void MTPattManager::setoline(int value)
{
	...
	if (patt){
		if (value>patt->nlines-1-nu) value = patt->nlines-1-nu;
		if (value<-nu) value = -nu;
...
void MTPattManager::setsel(MTRect value)
{
	...
	else if (value.left>=patt->ntracks) value.left = patt->ntracks-1;
	if (value.top<0) value.top = 0;
	else if (value.top>=patt->nlines) value.top = patt->nlines-1;
	if (value.right<0) value.right = 0;
	else if (value.right>=patt->ntracks) value.right = patt->ntracks-1;
	if (value.bottom<0) value.bottom = 0;
	else if (value.bottom>=patt->nlines) value.bottom = patt->nlines-1;
	s = selstart;
	e = selend;
	pminmax(s,e);
...
void MTPattManager::setselstart(MTPoint value)
{
	...
	else if (value.x>=patt->ntracks) value.x = patt->ntracks-1;
	if (value.y<0) value.y = 0;
	else if (value.y>=patt->nlines) value.y = patt->nlines-1;
	s = selstart;
	e = selend;
	pminmax(s,e);
	selstart = value;
...
void MTPattManager::setselend(MTPoint value)
{
	...
	if (selecting){
		if (value.x<otrack) setotrack(value.x);
		else if (value.x>=ntracks+otrack) setotrack(value.x-ntracks+1);
		if (value.y<oline) setoline(value.y);
		else if (value.y>=nlines+oline) setoline(value.y-nlines+1);
	};
	s = selstart;
	e = selend;
	ns = selstart;
	...
	pminmax(ns,ne);
	minx = s.x;
	if (ns.x<minx) minx = ns.x;
	maxx = e.x;
	if (ne.x>maxx) maxx = ne.x;
	miny = s.y;
	if (ns.y<miny) miny = ns.y;
	maxy = e.y;
	if (ne.y>maxy) maxy = ne.y;
	if (parent->parent){
		MTCMessage msg = {MTCM_CHANGE,0,parent};
		for (y=miny;y<=maxy;y++){
			for (x=minx;x<=maxx;x++){
		...
		if (pos.x<0) pos.x = -1;
			...
			for (pos.x=otrack;pos.x<patt->ntracks;pos.x++){
				if (x<cellwidth[pos.x]){
					...
					if (c==ti.ncolumns) c = -1;
					break;
				};
				x -= cellwidth[pos.x];
			};
			if (pos.x>=patt->ntracks) pos.x = -1;
		};
		pos.y = pos.y-patty;
		if (pos.y<0) pos.y = -1;
		else{
			pos.y = (pos.y/cellheight)+oline;
			if (pos.y>=patt->nlines) pos.y = -1;
		};
		return c;
	}
	else{
		...
		for (y=0;y<h;y++){
			...
			for (x=0,ct=t+otrack;x<w;x++,ct++){
				...
				for (n=0;n<ti.ncolumns;n++){				
					...
					if ((x+t+otrack==cursor.x) && (y+l+oline==cursor.y) && (ccol==n)) cstate.cursor = cpos;
					else cstate.cursor = -1;
