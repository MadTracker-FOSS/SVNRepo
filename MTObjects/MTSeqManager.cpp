...
bool MTSeqManager::onmessage(MTCMessage &msg)
{
	...
	case MTCM_MOUSEDOWN:
		...
		if (msg.buttons & DB_LEFT){
			if (msg.x>=pattx){
				if (msg.y<patty){
					if (module) module->setpos(clienttodata(msg.x,0));
				}
				else{
					x = 0;
					if (msg.buttons & DB_CONTROL) x = 1;
					if (msg.buttons & DB_SHIFT) x = 2;
					selectsequence(hl.y,hl.x,x);
					if ((msg.buttons & DB_DOUBLE) && (module) && (hl.x>=0) && (hl.y>=0)){
						x = module->sequ[hl.y][hl.x].patt;
						if (x & 0x1000) mtinterface->editobject(A(module->apatt,Automation)[x & 0xFFF],false);
						else mtinterface->editobject(A(module->patt,Pattern)[x],false);
					};
				};
			};
		};
...
void MTSeqManager::setcursor(double value)
{
	...
	if ((value!=cursor) && (cparent)){
		...
		if (followsong){
			if (cursor<offsetx) setoffsetx(cursor-nbeats/2);
			if (cursor>offsetx+nbeats-nbeats/8) setoffsetx(cursor-nbeats/8);
		};
	};
}
...
{
	if (value!=offsetx) hs->setposition(value/zoom);
}

void MTSeqManager::setoffsety(int value)
{
	if (value!=offsety) vs->setposition(value);
}
...
double MTSeqManager::clienttodata(int x,int *layer)
{
	...
	x -= pattx;
	if ((x<0) || (x>=width-pattx)) return -1.0;
...
int MTSeqManager::datatoclient(double x,int *layer)
{
	...
	if ((line<0) || (line>=width-pattx)) return -1;
	line += pattx;
	if (layer){
		*layer -= offsety;
		if ((*layer<0) || (*layer>=nlayers)){
...
void MTSeqManager::clienttosequence(int &x,int &layer)
{
	x -= pattx;
	layer -= patty;
	if ((!module) || (layer<0) || (x<0) || (x>=width-pattx)) goto error;
	layer /= patth;
	if (layer>=nlayers) goto error;
...
// Replace selection
	case 0:
		...
		if ((layer>=0) && (s>=0)) module->sequ[layer][s].flags |= SF_SELECTED;
		break;
// Toggle selection
	case 1:
		if ((layer>=0) && (s>=0)) module->sequ[layer][s].flags ^= SF_SELECTED;
...
void MTSeqManager::drawlayer(int sy,int sh)
{
	...
	if (sy+offsety>=MAX_LAYERS) return;
	if (sy<0){
		if (sy+sh>=0){
			sh += sy;
			sy = 0;
