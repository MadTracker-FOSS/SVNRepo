...
MTSample::~MTSample()
{
	if (filename) si->memfree(filename);
	splfree();
}
...
int MTSample::loadfromstream(MTFile *f,int size,void *params)
{
	...
	if ((nchannels>2) || (nchannels==0) || (depth==0)) return 0;
	if (!data[0]){
		if (!splalloc(size/(depth*nchannels))) return 0;
	};
	buffer = f->getpointer(-1,size);
	if (nchannels==1){
		memcpy(data[0],buffer,ns*depth);
		if (depth==1) a_delta_decode_8((char*)data[0],ns);
		else a_delta_decode_16((short*)data[0],ns);
	}
	else{
		memcpy(data[0],buffer,ns*depth);
...
{
	if ((nchannels==0) || (data[0]==0)) return 0;
...
panvarlng(0)
{
	...
	if (cparent.depth==1) divider *= 128;
	else if (cparent.depth==2) divider *= 32768;
	for (x=0;x<nstatus;x++){
...
bool MTSampleInstance::seek(double offset,int origin,int units)
{
	...
	if ((units & 0xF)==MTIS_BEATS) offset *= module->playstatus.spb;
	else if ((units & 0xF)==MTIS_MS) offset *= (double)module->playstatus.coutput->frequency/1000;
	if ((units & MTIS_INNER)==0) offset *= pitch;
	...
	case MTIS_CURRENT:
		...
		break;
	case MTIS_END:
		if ((!reverse) && ((offseti|offsetd)!=0)) return false;
		for (x=0;x<nstatus;x++){
...
void MTSampleInstance::setvolume(double vs,int steps,int curve)
{
	...
	if (steps){
		...
		if ((panvarlng>1) && (panvarlng<steps)) steps = panvarlng;
	}
	else{
		volvar = 0.0;
		volume = vs;
...
void MTSampleInstance::setpanning(float xs,float ys,float zs,int steps,int curve)
{
	...
	if (volvarlng){
		if (steps){
			if (volvarlng>steps) volvarlng = steps;
		}
		else{
			steps = volvarlng;
			for (x=0;x<nstatus;x++){
...
{
	if (!steps) return volume;
	if (steps>volvarlng) steps = volvarlng;
...
void MTSampleInstance::getpanning(float *x,float *y,float *z,int steps)
{
	...
	if (steps>panvarlng) steps = panvarlng;
	if (x) *x = panx+panxvar*steps;
	if (y) *y = pany+panyvar*steps;
	if (z) *z = panz+panzvar*steps;
}
